...
class TestEncoderDecoder(TestCase):
    def setUp(self):
        self.train_args = Args(src="test/data/nmt.en", trg="test/data/nmt.ja", model_out="/tmp/chainn/nmt/model/encdec", model="encdec",\
                hidden=100, epoch=5, embed=100, depth=2)
        self.test_args = Args(init_model=["/tmp/chainn/nmt/model/encdec"])
